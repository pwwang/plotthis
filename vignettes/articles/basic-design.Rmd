...
When `split_by` is used, `combine_plots()` **row-binds** the individual data frames from each sub-plot and adds a column named after the `split_by` variable, with values identifying which split each row came from:
...
table(p$data[["Species"]])   # one row per split level
```

This makes it straightforward to verify what data went into each sub-plot, or to extract the combined data for further analysis:

```r
# Inspect the data for a specific split level
subset(p$data, Species == "setosa")
```

Note that when `split_by` is used, the `p$data` is **reconstructed** from the individual sub-plot data — it may differ from the original input data due to per-split processing (`keep_na`, `keep_empty`, aggregation, etc.).
