...
AreaPlotAtomic <- function(
    ...
    p <- ggplot(data, aes(x = as.numeric(!!sym(x)), y = !!sym(y), fill = !!sym(fill_by))) +
...
#' )
#' AreaPlot(data, x = "x", y = "y", fill_by = "fill")
#' AreaPlot(data, x = "x", y = "y", fill_by = "fill",
#'          scale_y = TRUE)
#' AreaPlot(data, x = "x", y = "y", split_by = "fill")
AreaPlot <- function(
