...
# #'
# #' Support string of global functions for transformation, such as log2, log10, sqrt, etc
# #' and their negative counterparts, such as -log2, -log10, -sqrt, etc.
# #' And support functions given directly, such as function(x) log2(x + 1), function(x) -log10(x + 1), etc.
# #' @param trans A string or a function for transformation
# #' @param trans_name A string for the name of the transformation, used for labeling
# #' @param null_as_identity A logical value indicating whether to treat NULL as identity transformation. Default: FALSE.
# #' @return a list containing the transformation function and its name
# #' @importFrom rlang %||%
# #' @keywords internal
# process_trans <- function(trans, trans_name = NULL, null_as_identity = FALSE) {
#     if (is.null(trans)) {
#         if (null_as_identity) {
#             return(list(trans = identity, name = "identity"))
#         } else {
#             return(list(trans = NULL, name = NULL))
#         }
#     } else {
#         if (is.character(trans)) {
#             trans_name <- trans_name %||% trans
#             if (startsWith(trans, "-")) {
#                 trans_fun <- function(x) -match.fun(trimws(substring(trans, 2)))(x)
#             } else {
#                 trans_fun <- match.fun(trimws(trans))
#             }
#         } else if (is.function(trans)) {
#             trans_fun <- trans
#             trans_name <- trans_name %||% "transformed"
#         } else {
#             stop("`trans` must be a string, a function, or NULL", call. = FALSE)
#         }
#         return(list(trans = trans_fun, name = trans_name))
#     }
# }
