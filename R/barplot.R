...
#' @importFrom ggplot2 aes geom_bar scale_fill_manual labs scale_x_discrete scale_y_continuous element_line waiver
...
BarPlotAtomic <- function(
    ...
    if (is.null(group_by)) {
        ...
        p <- BarPlotGrouped(
            data, x, y, group_by, facet_by = facet_by,
            theme = theme, theme_args = theme_args, palette = palette, palcolor = palcolor,
            alpha = alpha, x_text_angle = x_text_angle, aspect.ratio = aspect.ratio,
...
BarPlot <- function(
    data, x, y, fill_by_x_if_no_group = TRUE,
