...
#' @param from_by A character string specifying the column name of the links data frame for the source nodes.
#'  Default is the first column of the links data frame.
#' @param from_by_sep A character string to concatenate the columns in `from_by`, if multiple columns are provided.
#' @param to_by A character string specifying the column name of the links data frame for the target nodes.
#'  Default is the second column of the links data frame.
#' @param to_by_sep A character string to concatenate the columns in `to_by`, if multiple columns are provided.
...
#' @param link_color_by A character string specifying the color of the links.
#'  This can be a color name, a hex code, or a column name from the links data frame.
...
#' @param node_palette A character string specifying the palette of the nodes.
#' @param node_palcolor A character vector specifying the colors of the node palette.
...
NetworkAtomic <- function(
    links, nodes = NULL, from_by = NULL, from_by_sep = "_", to_by = NULL, to_by_sep = "_",
    node_by = NULL, node_by_sep = "_", link_weight_by = 2, link_weight_name = NULL,
    link_color_by = "grey40", link_color_name = NULL, link_type_by = "solid", link_type_name = NULL,
    ...
    if (is.null(from_by)) {
        from_by <- "from"
        if (!from_by %in% colnames(links)) {
            colnames(links)[1] <- from_by
        }
    }
    if (is.null(to_by)) {
        to_by <- "to"
        if (!to_by %in% colnames(links)) {
            colnames(links)[2] <- to_by
        }
    }
    ...
    from_by <- check_columns(links, from_by, force_factor = TRUE, allow_multi = TRUE,
        concat_multi = TRUE, concat_sep = from_by_sep)
    to_by <- check_columns(links, to_by, force_factor = TRUE, allow_multi = TRUE,
        concat_multi = TRUE, concat_sep = to_by_sep)
    links <- links %>%
        rename(from = from_by, to = to_by) %>%
    ...
    if (cluster != "none") {
        ...
        p <- p + mark_fun(
            ...
            scale_fill_manual(values = palette_this(levels(df_nodes$cluster), palette = node_palette, palcolor = node_palcolor)) +
            scale_color_manual(values = palette_this(levels(df_nodes$cluster), palette = node_palette, palcolor = node_palcolor)) +
            new_scale_fill() +
            new_scale_color()
    }
    ...
    if (!link_color_by %in% colnames(df_edges)) {
...
Network <- function(
    ...
    plots <- lapply(names(linkss), function(nm) {
        ...
        NetworkAtomic(
            links = linkss[[nm]], nodes = nodes, from_by = from_by, from_by_sep = from_by_sep,
            to_by = to_by, to_by_sep = to_by_sep, node_by = node_by, node_by_sep = node_by_sep,
            link_weight_by = link_weight_by, link_weight_name = link_weight_name,
            link_color_by = link_color_by, link_color_name = link_color_name,
            ...
            link_curvature = link_curvature, link_palette = link_palette, link_palcolor = link_palcolor,
            node_palette = node_palette, node_palcolor = node_palcolor, directed = directed,
            layout = layout, cluster = cluster, add_mark = add_mark, mark_expand = mark_expand,
